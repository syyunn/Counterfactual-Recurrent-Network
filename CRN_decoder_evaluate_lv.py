...
def fit_CRN_decoder(
    ...
    num_inputs = (
        dataset_train["current_covariates"].shape[-1]
        + dataset_train["current_treatments"].shape[-1]
    )
    ...
    if b_hyperparam_opt:
        ...
        for simulation in range(num_simulations):
            logging.info(
                "Simulation {} out of {}".format(simulation + 1, num_simulations)
            )
            ...
            hyperparams["fc_hidden_units"] = int(
                np.random.choice([0.5, 1.0, 2.0, 3.0, 4.0]) * (hyperparams["br_size"])
            )
...
def process_counterfactual_seq_test_data(
    test_data, data_map, states, projection_horizon
):
    ...
    for i in range(num_patient_points):
        ...
        seq2seq_current_covariates[i] = np.repeat(
            [current_covariates[i, seq_length - 1]], projection_horizon, axis=0
        )
...
def test_CRN_decoder(
    pickle_map,
    max_projection_horizon,
    projection_horizon,
    models_dir,
    encoder_model_name,
    encoder_hyperparams_file,
    ...
    training_seq_processed = process_seq_data(
        training_processed, training_br_states, max_projection_horizon
    )  # br means balancing representation
