...
class CRN_Model:
    def __init__(self, params, hyperparams, b_train_decoder=False):
        ...
        self.current_covariates = tf.placeholder(
            tf.float32, [None, self.max_sequence_length, self.num_covariates]
        )

        # Initial previous treatment needs to consist of zeros (this is done when building the feed dictionary)
        self.previous_treatments = tf.placeholder(
            tf.float32, [None, self.max_sequence_length, self.num_treatments]
        )
        self.current_treatments = tf.placeholder(
            tf.float32, [None, self.max_sequence_length, self.num_treatments]
        )
        self.outputs = tf.placeholder(
            tf.float32, [None, self.max_sequence_length, self.num_outputs]
        )
        self.active_entries = tf.placeholder(
            tf.float32, [None, self.max_sequence_length, self.num_outputs]
        )
    ...
    def build_balancing_representation(self):
        ...
        balancing_representation = tf.layers.dense(
            rnn_output, self.br_size, activation=tf.nn.elu
        )
    ...
    def build_outcomes(
        self,
        balancing_representation,
    ):
        current_treatments_reshape = tf.reshape(
            self.current_treatments, [-1, self.num_treatments]
        )
    ...
    def train(self, dataset_train, dataset_val, model_name, model_folder):
        ...
        self.treatment_prob_predictions = self.build_treatment_assignments_one_hot(
            self.balancing_representation
        )
        ...
        self.loss_outcomes = self.compute_loss_predictions(
        ...
        for epoch in range(self.num_epochs):
            ...
            alpha_current = 2.0 / (1.0 + np.exp(-10.0 * p)) - 1
        ...
        (
            validation_loss,
            validation_loss_outcomes,
            validation_loss_treatments,
        ) = self.compute_validation_loss(dataset_val)
    ...
    def build_feed_dictionary(
        ...
        new_batch_previous_treatments = np.concatenate(
            [zero_init_treatment, batch_previous_treatments], axis=1
        )
    ...
    def gen_epoch(self, dataset, batch_size, training_mode=True):
        ...
        for i in range(num_batches):
            ...
            if training_mode:
                batch_current_covariates = dataset["current_covariates"][
                    batch_samples, :, :
                ]
                batch_previous_treatments = dataset["previous_treatments"][
                    batch_samples, :, :
                ]
                batch_current_treatments = dataset["current_treatments"][
                    batch_samples, :, :
                ]
    ...
    def compute_validation_loss(self, dataset):
        ...
        for (
            ...
            batch_outputs,
            batch_active_entries,
        ) in self.gen_epoch(dataset, batch_size=batch_size):
            feed_dict = self.build_feed_dictionary(
                batch_current_covariates,
                batch_previous_treatments,
                batch_current_treatments,
    ...
    def get_balancing_reps(self, dataset):
        ...
        for (
            ...
            for sample in range(num_samples):
                br_outputs = self.sess.run(
                    self.balancing_representation, feed_dict=feed_dict
                )
                br_outputs = np.reshape(
                    br_outputs, newshape=(-1, self.max_sequence_length, self.br_size)
                )
    ...
    def get_autoregressive_sequence_predictions(
        ...
        current_dataset["init_state"] = np.zeros(
            (num_patient_points, encoder_states.shape[-1])
        )
    ...
    def compute_loss_treatments_one_hot(
        self, target_treatments, treatment_predictions, active_entries
    ):
    ...
    def compute_loss_predictions(self, outputs, predictions, active_entries):
        predictions = tf.reshape(
            predictions, [-1, self.max_sequence_length, self.num_outputs]
        )
        mse_loss = tf.reduce_sum(
    ...
    def evaluate_predictions(self, dataset):
        ...
        unscaled_predictions = (
            predictions * dataset["output_stds"] + dataset["output_means"]
        )
        unscaled_predictions = np.reshape(
            unscaled_predictions,
            newshape=(-1, self.max_sequence_length, self.num_outputs),
        )
        ...
        mse = self.get_mse_at_follow_up_time(
            unscaled_predictions, unscaled_outputs, active_entries
        )
    ...
    def get_mse_at_follow_up_time(self, prediction, output, active_entires):
        mses = np.sum(
            np.sum((prediction - output) ** 2 * active_entires, axis=-1), axis=0
    ...
    def save_network(self, tf_session, model_dir, checkpoint_name):
        ...
        save_path = saver.save(
            tf_session, os.path.join(model_dir, "{0}.ckpt".format(checkpoint_name))
        )
